<?php
class Core_DataList_Column_InputLimit extends Core_DataList_Column_Abstract
{
	public $name;
	...
	public $inputHtmlOptions = array('class'=>'text-area-limit', 'size'=>5);
	...
	public function renderHeaderCellContent(){
		...
		if($this->grid->dataProvider->getItemCount()>0 && !empty($this->primary) && !empty($this->updateUrl)){		
			$this->linkHtmlOptions['onClick'] = "jQuery.fn.DataList.post('{$this->grid->id}', {url: '{$this->updateUrl}',data: jQuery('#{$this->grid->id} input.text-area-limit').serialize()});";
